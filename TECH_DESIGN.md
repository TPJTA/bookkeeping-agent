...
| 15 | 识别空项补全 | **卡片内选填表单,直接写回 Bitable** | 仅为识别为空的商户/商品/金额显示输入框;保留确认按钮且不校验表单是否填写;提交不调用 LLM,旧卡标记已补全并另发新待确认卡 |
...
用户点击「提交补全」后,程序忽略仍为空的输入值,并且只允许写入原本为空的字段。非空金额必须能解析为数字。更新直接写入 Bitable,不调用 LLM;成功后原卡片变为「已补全」无按钮状态,机器人回复一张新的待确认卡片。新卡片继续支持确认、撤销、自然语言修改,若仍有空项也继续展示补全表单。
