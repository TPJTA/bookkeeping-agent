...
- ✏️ **自然语言修改** —— 回复卡片「金额改成 50」「类别改为交通出行」,LLM 合并后旧卡片标记「🚫 已失效」,新卡片挂在你的修改消息下面
- 📝 **空项快捷补全** —— 商户、商品或金额识别为空时,待确认卡片只展示对应的选填输入框;提交后不调用 LLM,原卡片先显示「识别中」,直接更新记录后在原卡片展示待确认结果
...
- ✅ 在多维表格里**手动建好这些字段**:
   `商户`(文本)/ `商品`(文本)/ `类别`(单选:餐饮/超市生鲜/交通出行/居住缴费/旅行住宿/护肤美妆/服饰日用/游戏娱乐/数字服务/医疗健康/人情往来/其他)/ `金额`(数字)/ `状态`(单选:待确认/已确认)/ `置信度`(数字)/ `录入时间`(日期)/ `确认时间`(日期)/ `用户`(人员)/ `截图`(附件)/ `来源`(单选:飞书/快捷方式)
...
也可以用 Node.js 18+ 的 `fetch` 测试脚本模拟快捷方式上传:

```bash
node scripts/fetch_webhook.mjs \
  /path/to/order-screenshot.png \
  oc_your_review_chat_id
```

脚本默认请求本机的 `http://127.0.0.1:8000/webhook/order-screenshot`;第三个参数可传入其他 webhook URL。HTTP `202` 表示服务已接收图片,后续处理结果以目标飞书会话中的卡片为准。
